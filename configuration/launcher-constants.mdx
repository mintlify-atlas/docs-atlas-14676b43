...
Runs the prediction loop `iterations` times. Each pass re-estimates the time of flight from the updated predicted target distance, converging toward a self-consistent solution:

1. Compute initial distance to target.
2. Look up initial time of flight.
3. For each iteration: predict new target position → recompute distance → recompute time of flight.
...
Use `turretTransform()` to retrieve the offset as a `Transform2d`, or `launcherFromRobot(robotPose)` to get the launcher's `Translation2d` in field coordinates.

## NetworkTables telemetry

`UpdateNT(Pose2d robot)` must be called each loop (it is called from the launcher subsystem's periodic). It publishes two entries under `/SmartDashboard/LiveLauncherData/`:
