...
A single **TalonFX** (CAN ID `HOOD_MOTOR_ID = 22`) with **PositionVoltage** control. The hall-effect sensor (`HALL_EFFECT_SENSOR_ID = 0`) on the analog input is used for homing.
...
`isApproachingTrench(Pose2d, ChassisSpeeds)` samples **10 lookahead poses** over **0.5 seconds** at the current velocity. If any sample places the turret within ±12 in (X) × ±25 in (Y) of a trench AprilTag (IDs 1, 6, 7, 12, 17, 22, 23, 28), the hood is forced to 0 and `isAtTarget()` returns false.
